import { createSqliteArticleRepository } from './article-repository';

describe('sqlite-article-store', () => {
  it('stores article', async () => {
    const sqliteArticleStore = await createSqliteArticleRepository(':memory:');
    const stored = await sqliteArticleStore.storeArticle({
      doi: 'test/article.1',
      xml: '<article></article>',
  ...
  it('stores article content and retrieves a specific processed article by ID', async () => {
    ...
    const result = await sqliteArticleStore.storeArticle(exampleArticle);
    expect(result).toStrictEqual(true);

    const article = await sqliteArticleStore.getArticle('test/article.2');
  ...
  it('errors when retrieving unknown article', async () => {
    const sqliteArticleStore = await createSqliteArticleRepository(':memory:');
    expect(async () => await sqliteArticleStore.getArticle('test/article.3')).rejects.toThrowError();
  });
  ...
  it('gets articles summaries', async () => {
    ...
    await sqliteArticleStore.storeArticle(exampleArticle1);
    await sqliteArticleStore.storeArticle(exampleArticle2);
    await sqliteArticleStore.storeArticle(exampleArticle3);

    const articleSummaries = await sqliteArticleStore.getArticleSummaries();
